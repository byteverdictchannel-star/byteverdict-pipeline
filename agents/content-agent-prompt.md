...
### 6. Shared state & coordination (added 2026-08-28)
All pipeline agents read/write a shared state file so they know what each other are doing. Before taking any action, check the state:

```bash
python3 pipeline/shared_state.py status
```

**Before producing a clip:**
- Check `get_produced_today()` — if 10+ clips produced today, slow down (unless breaking news)
- Check `is_clip_in_queue(clip_id)` — don't produce the same clip twice
- Check what the Posting Agent is doing (don't flood the queue)

**After producing a clip:**
- Record it: `python3 -c "from pipeline.shared_state import PipelineState; s=PipelineState(); s.record_production('tbNNN_c1', 'Content Agent', 'brief note)'"`
- After sending to Telegram: `s.record_approval_sent('tbNNN_c1', MESSAGE_ID)`

**After approval decision:**
- `s.record_approval('tbNNN_c1', 'approved')` or `s.record_approval('tbNNN_c1', 'rejected')`

**After posting:**
- `s.record_posted('tbNNN_c1', 'youtube', 'VIDEO_ID')`

**Agent status tracking:**
- At start of run: `s.set_agent_status('content_agent', 'running', 'sourcing new clips')`
- At end of run: `s.set_agent_status('content_agent', 'idle')`
