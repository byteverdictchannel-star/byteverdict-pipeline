...
sys.path.insert(0, str(SCRIPT_DIR.parent.parent.parent))  # pipeline/ — for watch_gate
...
try:
    from watch_gate import record_watch_receipt  # noqa: E402
except ImportError:
    # Vendored tool should keep working standalone even if watch_gate.py
    # (a ByteVerdict-specific addition, not part of the upstream tool)
    # isn't present for some reason — degrade to no receipt, not a crash.
    record_watch_receipt = None
...
def main() -> int:
    ...
    if record_watch_receipt is not None and frames and video_path:
        try:
            record_watch_receipt(video_path, len(frames), detail)
        except Exception as exc:
            # A receipt-write failure must never break the actual /watch
            # report the caller is waiting on — surface it, don't raise.
            print(f"[watch] warning: could not record watch receipt: {exc}", file=sys.stderr)
